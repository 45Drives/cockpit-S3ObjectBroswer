<template>
  ...
  >
    Cut
            ...
            <MenuItem v-slot="{ active, disabled }" :disabled="!canPaste">
            <button type="button" class="block w-full px-4 py-2 text-left text-sm" :disabled="!canPaste" :class="[
              !canPaste ? 'opacity-50 cursor-not-allowed text-muted' : (active ? 'bg-accent text-default' : 'text-muted')
...
export type MenuAction = "download" | "delete" | "rename" | "copy" | "paste" | "cut";
